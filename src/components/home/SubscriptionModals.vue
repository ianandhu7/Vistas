...
const handleVerify = async () => {
  const wasHeroFlow = store.isHeroFlow
  await store.verifyOtpAndCheckout()
  if (wasHeroFlow && !store.error) {
    router.push('/explore')
  }
}
        ...
        <div class="space-y-8">
          <div class="flex justify-between gap-2">
            <input 
              ...
              @input="store.setOtpDigit(i-1, $event.target.value.replace(/\D/g, '')); handleOtpInput($event, i-1)"
          ...
          <div class="text-center">
            ...
            <button v-else @click="handleResendOtp()" class="text-[11px] text-[var(--accent-purple)] font-black hover:underline">Resend OTP Now</button>
          </div>
        </div>
      </div>
